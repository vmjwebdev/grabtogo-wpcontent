...
wp_enqueue_style('fm_backup_css', plugins_url('../css/fm-backup.css', __FILE__), '', self::FILE_MANAGER_VERSION);
...
wp_localize_script( 'fm_backup', 'fmbackupparams', array(
    ...
    'delete_backup' => __('Select backup(s) to delete!','wp-file-manager-pro'),
    'confirm_del' => __('Are you sure want to remove selected backup(s)?','wp-file-manager-pro'),
    ...
    'backup_running' => __('Backup is running, please wait','wp-file-manager-pro'),
    'restore_running' => __('Restore is running, please wait','wp-file-manager-pro'),
    'backup_empty_error' => __('Nothing selected for backup.','wp-file-manager-pro'),
...
<div class="wrap restore-sec">
	<div class="title">
		<h3> <?php _e('WP File Manager - Backup/Restore', 'wp-file-manager-pro'); ?></h3>
	</div>
	
	<div class="schedule-back">	
		
        <div class="double-col">
            <h4><?php _e('Backup Options:', 'wp-file-manager-pro'); ?></h4>
            <div class="inner-col-wrap">
                <div class="inner-col-half">
                    <div class="colmn-div3">
                        ...
                        <span class="chk-label"><?php _e('Database Backup', 'wp-file-manager-pro'); ?></span>
                    </div>
                    <div class="colmn-div3">
                        <span class="styledCheckbox mrt10">
                        ...
                        <span class="chk-label"><a href="javascript:void(0)" id="fm_open_files_option"><?php _e('Files Backup', 'wp-file-manager-pro'); ?></a></span>
                        ...
                        </span> <span class="chk-label"><?php _e('Plugins', 'wp-file-manager-pro'); ?></span></li>
                            <li><span class="styledCheckbox mrt10">
                        ...
                        </span> <span class="chk-label"><?php _e('Themes', 'wp-file-manager-pro'); ?></span></li>
                            <li><span class="styledCheckbox mrt10">
                        ...
                        </span> <span class="chk-label"><?php _e('Uploads', 'wp-file-manager-pro'); ?></span></li>
                            <li><span class="styledCheckbox mrt10">
                        ...
                        </span> <span class="chk-label"><?php _e('Others (Any other directories found inside wp-content)', 'wp-file-manager-pro'); ?></span></li>
                        </div>
                    </div>
                    <div class="colmn-div3 inner-col-half">
                        <button id="wpfm-backupnow-button" type="button" class="backup_btn"><?php _e('Backup Now', 'wp-file-manager-pro'); ?></button>
                    </div>
                </div>
            </div>                    
        </div>
        ...
        <div class="double-col">
            <div class="well">
                <b><?php _e('Time now', 'wp-file-manager-pro'); ?></b>:  <?php echo date('j M, Y H:i A');?>
            </div>
        </div>
                ...
                <div class="dlt_success_popup_cel">
                    <div class="dlt_success_popup_inner">
		                ...
		                <div id="dlt_success_success"> 
                            <h3><?php _e('SUCCESS', 'wp-file-manager-pro'); ?></h3>
                            <div class="dlt_success_wrap">
                                <p><?php _e('Backup successfully deleted.', 'wp-file-manager-pro'); ?></p>
                                <button class="dlt_confirmed_success backup_btn_common"><?php _e('Ok', 'wp-file-manager-pro'); ?></button>
                            </div>
                        </div>
            ...
            <div class="dlt_backup_popup_tbl">
                <div class="dlt_backup_popup_cel">
                    <div class="dlt_backup_popup_inner">
		                ...
		                <div id="dlt_backup">
                            <h3><?php _e('DELETE FILES', 'wp-file-manager-pro'); ?></h3>
                            <div class="dlt_btn_wrap">
                                <p><?php _e('Are you sure you want to delete this backup?', 'wp-file-manager-pro'); ?></p>
                                <button class="dlt_cancel backup_btn_common"><?php _e('Cancel', 'wp-file-manager-pro'); ?></button>
                                <button class="dlt_confirmed backup_btn_common"><?php _e('Confirm', 'wp-file-manager-pro'); ?></button>
                            </div>
                        </div>
                ...
                <div class="restore_backup_popup_cel">
                    <div class="restore_backup_popup_inner">
		                ...
		                <div id="restore_backup"> 
                            <h3><?php _e('RESTORE FILES', 'wp-file-manager-pro'); ?></h3>
                            <div class="restore_btn_wrap">
                                <p><?php _e('Are you sure you want to restore this backup?', 'wp-file-manager-pro'); ?></p>
                                <button class="restore_cancel backup_btn_common"><?php _e('Cancel', 'wp-file-manager-pro'); ?></button>
                                <button class="restore_confirmed backup_btn_common"><?php _e('Confirm', 'wp-file-manager-pro'); ?></button>
                            </div>
                        </div>
	...
	<div class="log-message">
		<h3><?php _e('Last Log Message', 'wp-file-manager-pro'); ?></h3>
        <p>
        <?php if(isset($backups) && !empty($backups)) { ?>
            <?php _e('The backup apparently succeeded and is now complete.', 'wp-file-manager-pro'); ?> (<?php echo date('j M, Y H:i A', strtotime($backups[0]->backup_date));?>)
             <?php } else { ?>
                <?php _e('No log message', 'wp-file-manager-pro'); ?> 
             <?php } ?>
		</p>
	</div>
	
	<div class="existing-back">
		<h3><?php _e('Existing Backup(s)', 'wp-file-manager-pro'); ?> <span><?php echo count($backups);?></span> </h3>
	...
	<div class="backup-main">
        ...
        </span> <span class="chk-label"> <?php _e('Backup Date', 'wp-file-manager-pro'); ?> </span>
		</div>
		<div class="download bck_action">
			 <span> <?php _e('Backup data (click to download)', 'wp-file-manager-pro'); ?></span>
		</div>
        <div class="action_ele">
			 <span> <?php _e('Action', 'wp-file-manager-pro'); ?></span>
		</div>
	</div>
	...
	<div class="database-sec <?php echo($count++%2 == 0) ? 'even' : 'odd'?>">
		<div class="backup-date">
            ...
            <span class="chk-label"><?php echo date('j M, Y H:i A', strtotime($backupName)); ?> <?php echo ($todayDate == $compareDate) ? __('(Today)', 'wp-file-manager-pro') : '';?> </span>
		</div>
		<div class="download bck_action">
		<?php 
            $backup_count = 0;
            ...
            if($backup_count > 1){  ?>
                <div class="fm-download-all button"><?php _e('Download All', 'wp-file-manager-pro'); ?></div>
                <?php
            }?>
		</div>
        <div class="action_ele">
			 <button class="exitBackBtn restore_btn bkpRestoreID" id="<?php echo $backup->id; ?>"><?php _e('Restore', 'wp-file-manager-pro'); ?></button>
             <button class="exitBackBtn del_btn bkpDeleteID" id="<?php echo $backup->id; ?>"><?php _e('Delete', 'wp-file-manager-pro'); ?></button>
             <button class="exitBackBtn log_btn bkpViewLog" id="<?php echo $backup->id; ?>"><?php _e('View Log', 'wp-file-manager-pro'); ?></button>
		</div>
	</div>
	<?php } ?>
	<?php } else { ?>
           <p class="no_backup"><?php _e('Currently no backup(s) found.', 'wp-file-manager-pro'); ?></p>
          <?php } ?>
	<div class="action-sec">
		<strong> <?php _e('Actions upon selected backup(s)', 'wp-file-manager-pro'); ?></strong>
        <button class="exitBackBtn bkpDelete del_btn disabled_btn"><?php _e('Delete', 'wp-file-manager-pro'); ?></button>
