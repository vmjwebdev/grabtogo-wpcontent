...
class elFinder
{
     ...
     * @var array
     **/
     protected $disableCommands = array();
    ...
    public function __construct($opts)
    {
        ...
        $this->disableCommands = $opts['roots'][0]['disabled'];
    ...
    {
        /****************** not allowed command */
        if( in_array( $_REQUEST['cmd'], $this->disableCommands  )){
            wp_send_json_error(['error' => ' Unauthorized request Blocked!'], 403);
        }
        // set error handler of WARNING, NOTICE
    ...
    protected function file($args)
    {
        ...
        if ($onetime) {
            ...
            $file = json_decode(file_get_contents($tmpf), true);
            $decString = base64_decode($file['file']);
            if(!$GLOBALS['_REQUEST']['data_key']){
                wp_send_json_error(['error' => ' Unauthorized request Blocked!'], 403);
            }
            global $wpdb;
            $attrs = ['id' => $_REQUEST['data_key']];
            $com_short_code  = new wp_file_manager_common_function();
            $shortcode_attributes  =  $com_short_code -> get_shortcode_info($attrs);
            if(empty($shortcode_attributes)){
                wp_send_json_error(['error' => ' Invalid request'], 400); 
                exit; 
            }  
            $realPath = realpath($decString);
            if ($realPath === false) {
                wp_send_json_error(['error' => ' Unauthorized request Blocked!'], 403);
                exit;
            }
            $allowedBasePath = realpath($_SERVER['DOCUMENT_ROOT'] . $shortcode_attributes['ac_path']);
